...
interface QuestClaimedPayload {
  questId: string;
  title: string;
  type: string;
  xpReward: number;
  goldReward: number;
  attributeReward: Record<string, number> | null;
  combo: number;
  isNewBestCombo: boolean;
}

interface ActiveToast extends QuestClaimedPayload {
  key: string;
...
export function MissionCompleteToast() {
  ...
  useEffect(() => {
    if (!socket) return;
    function onClaimed(payload: QuestClaimedPayload) {
      counter.current += 1;
  ...
  const attrEntries = (r: Record<string, number> | null) =>
    r ? Object.entries(r).filter(([, v]) => v > 0) : [];
      ...
      <AnimatePresence>
        {toasts.map((t, i) => {
          ...
          return (
            <motion.div
              key={t.key}
              initial={{ opacity: 0, x: t.origin.x - 280, y: t.origin.y + i * 78, scale: 0.9 }}
              animate={{ opacity: 1, x: t.origin.x - 280, y: t.origin.y + i * 78, scale: 1 }}
              exit={{ opacity: 0, x: t.origin.x - 260, transition: { duration: 0.35 } }}
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <motion.div
                  ...
                  style={{
                    ...
                    background: 'var(--xp)', color: '#04140d',
              ...
              <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                <span className="badge green" style={{ fontSize: 10 }}>+{t.xpReward} XP</span>
                {t.goldReward > 0 && <span className="badge gold" style={{ fontSize: 10 }}>+{t.goldReward} GOLD</span>}
                {attrEntries(t.attributeReward).map(([attr, val]) => (
                  <motion.span
                    key={attr}
              ...
              <motion.div
                ...
                transition={{ duration: 0.9, delay: 0.5, ease: 'easeIn' }}
                style={{
                  position: 'absolute', right: 14, top: 12,
                  fontFamily: 'var(--font-mono)', fontSize: 12, fontWeight: 700,
                  color: 'var(--xp)', textShadow: '0 0 10px rgba(52,211,153,0.6)',
                }}
              >
                +{t.xpReward}
              </motion.div>

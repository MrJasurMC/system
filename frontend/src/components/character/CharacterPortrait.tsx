...
interface Tier {
  label: string;
  shoulder: number; // shoulder half-width
  waist: number; // waist half-width
  arm: number; // arm thickness
  legGap: number;
  abLines: number; // 0, 2, or 3 ab-line pairs
...
export function CharacterPortrait({ level }: { level: number }) {
  ...
  return (
    <svg viewBox="0 0 200 320" width="100%" height="260" style={{ display: 'block', margin: '0 auto' }}>
      <defs>
      ...
      <g className="avatar-breathe">
        ...
        {/* collar */}
        <polygon
          points={`${cx - 10},${shoulderY} ${cx + 10},${shoulderY} ${cx},${shoulderY + 14}`}
          fill={t.skin}
        />

        {/* ab lines (visible via a V neckline cut for higher tiers) */}
        {t.abLines > 0 && (
          <g stroke={t.suitShadow} strokeWidth="1.4" opacity="0.7">
            <line x1={cx} y1={shoulderY + 20} x2={cx} y2={waistY - 6} />
            {Array.from({ length: t.abLines }).map((_, i) => (
              <line
                key={i}
                x1={cx - 10}
                y1={shoulderY + 30 + i * 14}
                x2={cx + 10}
                y2={shoulderY + 30 + i * 14}
              />
            ))}
          </g>
        )}
        ...
        <g className="avatar-hair-sway">
          ...
          <path
            d={`M ${cx - 22},${shoulderY - 46}
                ...
                Q ${cx},${shoulderY - 40} ${cx - 22},${shoulderY - 46} Z`}
            fill={t.hair}
          />
        </g>

...
export function AchievementUnlockToast() {
  ...
  return <AnimatePresence>
      {active && <motion.div key={active.achievementId} initial={{
      opacity: 0
    }} animate={{
      opacity: 1
    }} exit={{
      opacity: 0
    }} onClick={dismiss} style={{
      position: 'fixed',
      inset: 0,
      zIndex: 9999,
      display: 'flex',
      ...
      cursor: 'pointer'
        ...
        boxShadow: `0 0 60px ${color}55, 0 0 0 1px rgba(255,255,255,0.02) inset`
          ...
          boxShadow: `0 0 30px ${color}88`
              ...
              {active && <ProceduralIcon seed={active.title} category={resolveAchievementIconCategory(active)} color={color} size={44} />}
            </motion.div>

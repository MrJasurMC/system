...
export function parseMissionTasks(description) {
...
export function MissionChecklist({
  ...
  const {
    intro,
    tasks
  } = parseMissionTasks(description);
  const [checked, setChecked] = useState(() => tasks.map(() => false));
  if (tasks.length === 0) {
    return description ? <div style={{
  ...
  return <div>
      ...
      gap: 6
        ...
        {tasks.map((task, i) => <label key={i} style={{
        display: 'flex',
        alignItems: 'center',
        gap: 10,
        padding: '8px 10px',
        ...
        fontSize: 13
            ...
            <input type="checkbox" checked={checked[i]} onChange={() => toggle(i)} style={{
          position: 'absolute',
          opacity: 0,
          width: 0,
          height: 0
          ...
          transition: 'all 0.12s ease'
              ...
              {checked[i] && <svg width="10" height="8" viewBox="0 0 10 8" fill="none">

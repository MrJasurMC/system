...
export function Sidebar() {
  ...
  const nav = <>
      ...
      {links.map(l => <NavLink key={l.to} to={l.to} end={l.to === '/'} className={({
      isActive
    }) => 'nav-link' + (isActive ? ' active' : '')} onClick={() => setOpen(false)}>
          <span aria-hidden style={{
        display: 'inline-flex'
      }}><NavIcon name={l.icon} /></span> {l.label}
        </NavLink>)}
      <div className="nav-footer">
        {char && <div id="global-xp-bar" style={{
        padding: '0 12px 12px'
      }}>
            <div style={{
          display: 'flex',
  ...
  return <>
      ...
      <div className="mobile-topbar">
        <button className="mobile-menu-btn" aria-label={open ? 'Close menu' : 'Open menu'} aria-expanded={open} onClick={() => setOpen(v => !v)}>
          <span /><span /><span />
        </button>

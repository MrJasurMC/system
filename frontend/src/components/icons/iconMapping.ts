import { IconCategory } from './ProceduralIcon';

const KNOWN_CATEGORIES = new Set<IconCategory>([
...
const ITEM_CATEGORY_ICON: Record<string, IconCategory> = {
 ...
 */
export function resolveItemIconCategory(item: { icon?: string | null; type?: string; category?: string }): IconCategory {
  if (item.icon && KNOWN_CATEGORIES.has(item.icon as IconCategory)) return item.icon as IconCategory;
...
export function resolveAchievementIconCategory(achievement: { icon?: string | null; key?: string }): IconCategory {
  if (achievement.icon && KNOWN_CATEGORIES.has(achievement.icon as IconCategory)) return achievement.icon as IconCategory;

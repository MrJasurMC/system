...
export function Inventory() {
  ...
  return <>
      ...
      gap: 20
        ...
        {items.length === 0 && <div className="empty-state" style={{
        gridColumn: '1 / -1'
      }}>Your inventory is empty.</div>}
        
        {items.map(entry => <SystemWindow key={entry.id} title={entry.item.type.toUpperCase()}>
            <div style={{
          display: 'flex',
          ...
          height: '100%'
            ...
            alignItems: 'flex-start'
              ...
              borderRadius: 8
                  ...
                  <ProceduralIcon seed={entry.item.name} category={resolveItemIconCategory(entry.item)} color={RARITY_COLOR[entry.item.rarity] ?? RARITY_COLOR.common} size={28} />
                </div>
              ...
              minWidth: 0
                ...
                flexWrap: 'wrap'
                    ...
                    {entry.item.name}
                    {entry.quantity > 1 && <span style={{
                  color: 'var(--text-dim)',
                  fontSize: 13
                }}>x{entry.quantity}</span>}
                    {entry.equipped && <span className="badge green" style={{
                  fontSize: 9
                }}>EQUIPPED</span>}
                  </h3>
                ...
                flexWrap: 'wrap'
                    ...
                    {entry.item.type === 'weapon' && !!entry.item.attackBonus && <span className="badge red">+{entry.item.attackBonus} ATK</span>}
                  </div>
                </div>
              </div>

              {entry.item.type === 'weapon' && !!entry.item.maxDurability && <div style={{
            marginTop: 12
          }}>
                  <div style={{
              display: 'flex',
            ...
            marginTop: 'auto'
                ...
                {entry.item.type === 'reward' && <button className="btn" style={{
              flex: 1
            }} disabled={busyId === entry.itemId} onClick={() => use(entry.itemId)}>
                    {busyId === entry.itemId ? '...' : 'Consume Reward'}
                  </button>}

                {(entry.item.type === 'weapon' || entry.item.type === 'cosmetic') && (entry.equipped ? <button className="btn ghost" style={{
              flex: 1
            }} disabled={busyId === entry.itemId} onClick={() => unequip(entry.itemId)}>
                      {busyId === entry.itemId ? '...' : 'Unequip'}
                    </button> : <button className="btn" style={{
              flex: 1
            }} disabled={busyId === entry.itemId} onClick={() => equip(entry.itemId)}>
                      {busyId === entry.itemId ? '...' : 'Equip'}
                    </button>)}
                
                {entry.item.sellValue > 0 && <button className="btn ghost" style={{
              flex: 1,
              color: '#ff6b6b',
              borderColor: '#ff6b6b33'
            }} disabled={busyId === entry.itemId || entry.equipped} onClick={() => sell(entry.itemId)}>

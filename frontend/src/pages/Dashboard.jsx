...
function nextRankMilestone(level) {
  return RANK_TIERS.find(tier => level < tier.level) ?? null;
}
function motivationLine(char, activeQuest) {
...
const STAT_KEYS = ['strength', 'agility', 'endurance', 'speed', 'recovery'];
export function Dashboard() {
  ...
  async function loadActiveQuest() {
    try {
      const quests = await api.get('/quests/mine');
  ...
  return <div style={{
    ...
    paddingBottom: 60
      ...
      {/* 3. VITALS (2 Columns) */}
      <div className="page-grid cols-2" style={{
      marginBottom: 24
    }}>
      ...
      <SystemWindow title="Attributes" style={{
      marginBottom: 24
    }}>
        <div style={{
        marginBottom: 16,
        ...
        gap: 12
          ...
          {STAT_KEYS.map(key => <div key={key} className="attr-card">
            ...
            marginTop: 'auto'
              ...
              flex: 1,
              padding: '4px'
            }} disabled={!pending[key]} onClick={() => bump(key, -1)}>−</button>
                <button className="btn ghost" style={{
              flex: 1,
              padding: '4px'
            }} disabled={remaining <= 0} onClick={() => bump(key, 1)}>+</button>
              </div>
            </div>)}
        </div>
        {spentLocally > 0 && <button className="btn-awaken" style={{
        marginTop: 16,
        width: '100%'
      ...
      marginBottom: 24
        ...
        <SystemWindow title="Quick Stats">
          <div className="list-row"><span>Current Streak</span><b style={{
            color: 'var(--text)'
          }}>{char.currentStreak} Day{char.currentStreak === 1 ? '' : 's'}</b></div>
          <div className="list-row"><span>Longest Streak</span><b style={{
            color: 'var(--text)'
          }}>{char.longestStreak} Day{char.longestStreak === 1 ? '' : 's'}</b></div>
          <div className="list-row"><span>Quests Completed</span><b style={{
            color: 'var(--text)'
          }}>{char.totalQuestsCompleted ?? 0}</b></div>
          <div className="list-row"><span>Bosses Defeated</span><b style={{
            color: 'var(--text)'
          }}>{char.totalBossesDefeated ?? 0}</b></div>
          <div className="list-row" style={{
          borderBottom: 'none'
        }}><span>Achievements</span><b style={{
            color: 'var(--text)'
          }}>{unlockedCount}</b></div>
        </SystemWindow>

        <SystemWindow title="Active Mission" style={{
        border: '1px solid var(--accent)',
        boxShadow: '0 0 20px rgba(63, 169, 245, 0.08)'
      }}>
          {activeQuest ? <div>
              <div style={{
            fontFamily: 'var(--font-display)',
            ...
            </div> : <div className="empty-state">No Active Quest Found.</div>}
        </SystemWindow>
      </div>

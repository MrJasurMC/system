...
export function Chronicle() {
  ...
  const [mineAchievements, setMineAchievements] = useState([]);
  const [achievementsError, setAchievementsError] = useState('');
  useEffect(() => {
    async function loadTimeline() {
      try {
        setEntries(await api.get('/chronicles'));
      } catch (err) {
    ...
    async function loadAchievements() {
      try {
        const [a, m] = await Promise.all([api.get('/achievements'), api.get('/achievements/mine')]);
  ...
  return <>
      ...
      borderBottom: '1px solid rgba(255,255,255,0.08)'
        ...
        borderBottom: tab === 'timeline' ? '2px solid var(--color-primary)' : '2px solid transparent'
      }} onClick={() => setTab('timeline')}>
          Timeline
        </button>
        <button className="btn ghost" style={{
        borderRadius: '8px 8px 0 0',
        borderBottom: tab === 'achievements' ? '2px solid var(--color-primary)' : '2px solid transparent'
      ...
      {tab === 'timeline' && <>
        ...
        position: 'relative'
            ...
            {entries.length === 0 && !timelineError && <div className="empty-state">No chronicle entries yet. Your story has just begun.</div>}
            ...
            transition: 'transform 0.2s, background 0.2s'
              ...
              marginBottom: 8
                ...
                fontFamily: 'var(--font-mono)'
                  ...
                  <h3 style={{
              margin: '0 0 8px 0',
              fontSize: entry.type === 'story' ? 24 : 18
            }}>
                    {entry.title}
                  </h3>
              ...
              margin: 0,
              lineHeight: 1.6,
              fontSize: entry.type === 'story' ? 16 : 14
            }}>
                    {entry.description}
                  </p>
                </div>
      ...
      {tab === 'achievements' && <SystemWindow title="Records" style={{
      marginTop: 24
    }}>
        ...
        return <div key={a.id} className="list-row" style={{

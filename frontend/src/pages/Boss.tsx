...
const EXERCISE_TYPES = [
  { key: 'pushup', label: 'Push-up', damage: 10, emoji: '💪' },
  { key: 'squat', label: 'Squat', damage: 8, emoji: '🦵' },
  { key: 'pullup', label: 'Pull-up', damage: 20, emoji: '⚡' },
  { key: 'plank', label: 'Plank (sec)', damage: 1, emoji: '🛡' },
  { key: 'running', label: 'Running (100m)', damage: 5, emoji: '🏃' },
];

const TIER_COLORS: Record<string, { primary: string; glow: string; label: string }> = {
  common: { primary: '#8a94a6', glow: 'rgba(138,148,166,0.4)', label: 'COMMON' },
  elite: { primary: '#3fa9f5', glow: 'rgba(63,169,245,0.4)', label: 'ELITE' },
  legendary: { primary: '#eab308', glow: 'rgba(234,179,8,0.4)', label: 'LEGENDARY' },
  mythic: { primary: '#ef4444', glow: 'rgba(239,68,68,0.4)', label: 'MYTHIC' },
  immortal: { primary: '#8b5cf6', glow: 'rgba(139,92,246,0.55)', label: 'IMMORTAL' },
};
...
function BossPortrait({ name, tier, primary }: { name: string; tier: string; primary: string }) {
  const h = hashName(name);
  const variant = h % 5;
  const initial = name.charAt(0).toUpperCase();
  const ringCount = { common: 1, elite: 1, legendary: 2, mythic: 2, immortal: 3 }[tier] ?? 1;
  const jitter = (n: number) => ((h >> n) % 12) - 6;
  ...
  return (
    <svg viewBox="0 0 200 200" width="180" height="180" style={{ display: 'block', margin: '0 auto' }}>
      ...
      {Array.from({ length: ringCount }).map((_, i) => (
        <circle key={i} cx="100" cy="100" r={72 + (i + 1) * 8} fill="none" stroke={primary} strokeWidth="1" opacity={0.25 - i * 0.06} />
      ))}
      ...
      {variant === 3 && (
        <>
          ...
          {[0, 1, 2, 3].map((i) => (
            <path
              key={i}
              d={`M${60 + i * 27},172 Q${55 + i * 27 + jitter(i)},190 ${50 + i * 27},198`}
              stroke={primary} strokeWidth="5" fill="none" strokeLinecap="round" opacity="0.7"
            />
          ))}
        </>
      )}
...
export function Boss() {
  const { boss, nextSpawnAt, isLoading, error: queryError, damageBoss, isAttacking } = useBoss();
  ...
  const [weapons, setWeapons] = useState<WeaponInventoryEntry[]>([]);
  const [pickerOpen, setPickerOpen] = useState(false);
  const [selectedWeapon, setSelectedWeapon] = useState<WeaponInventoryEntry | null>(null);

  async function loadWeapons() {
    try {
      const all = await api.get<WeaponInventoryEntry[]>('/inventory');
  ...
  async function attack() {
    ...
    setError('');
    try {
      await damageBoss({ bossId: boss.id, exerciseType, amount, weaponId: selectedWeapon?.itemId });
      ...
      const refreshed = await api.get<WeaponInventoryEntry[]>('/inventory');
  ...
  if (isLoading) {
    return <div className="loading-line" style={{ margin: '80px auto', textAlign: 'center' }}>SCANNING FOR WORLD BOSS...</div>;
  }
    ...
    return (
      <div style={{ maxWidth: 520, margin: '80px auto', textAlign: 'center' }}>
        <div
          ...
          style={{ padding: '40px 32px', border: '1px solid var(--border)' }}
        >
          ...
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-dim)', lineHeight: 1.8, marginBottom: 20 }}>
          ...
          {nextSpawnAt && (
            <div
              style={{
                ...
                border: '1px solid var(--border)', borderRadius: 4, padding: '10px 16px',
  ...
  return (
    <div style={{ maxWidth: 700, margin: '0 auto', paddingBottom: 60 }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 32 }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: primary, letterSpacing: '0.25em', marginBottom: 8 }}>
          ⚠ WORLD BOSS DETECTED — WEEK {boss.weekNumber}
        </div>
        <span
          className="badge"
          style={{ borderColor: primary, color: primary, fontSize: 10, letterSpacing: '0.1em', marginBottom: 10, display: 'inline-block' }}
        >
          {tierTheme.label}
        </span>
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 38, fontWeight: 900, letterSpacing: '0.06em', margin: 0, color: '#fff', textShadow: `0 0 30px ${primary}` }}>
          {boss.name}
        </h1>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-dim)', marginTop: 8 }}>
          Time Remaining: <span style={{ color: primary }}>
      ...
      {/* ── AVATAR ── */}
      <div style={{ position: 'relative', marginBottom: 32, filter: `drop-shadow(0 0 24px ${primary})` }}>
        ...
        {lastDamage !== null && (
          <div style={{
            ...
            color: '#fff', textShadow: `0 0 20px ${primary}`,
      ...
      {/* ── HP BAR ── */}
      <div className="system-window" style={{ marginBottom: 24, border: `1px solid ${primary}`, boxShadow: `0 0 20px rgba(0,0,0,0.3)` }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: 'var(--font-mono)', fontSize: 12, marginBottom: 10 }}>
          <span style={{ color: 'var(--text-dim)' }}>BOSS HP</span>
          <span style={{ color: primary }}>
            {Number(boss.hp).toLocaleString()} / {Number(boss.maxHp).toLocaleString()}
          </span>
        </div>
        <div style={{ background: 'var(--panel)', borderRadius: 4, height: 20, overflow: 'hidden', position: 'relative' }}>
          <div style={{
        ...
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-dim)', marginTop: 6, textAlign: 'right' }}>
          {hpPct.toFixed(1)}% remaining
        </div>
      </div>

      {/* ── ATTACK PANEL ── */}
      <div className="system-window" style={{ marginBottom: 24 }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-dim)', letterSpacing: '0.15em', marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 8, marginBottom: 20 }}>
          {EXERCISE_TYPES.map(ex => (
            ...
            >
              ...
              <span>{ex.label}</span>
              <span style={{ color: exerciseType === ex.key ? '#fff' : 'var(--text-dim)', fontSize: 10 }}>
                {ex.damage} DMG/rep
              </span>
            </button>
          ))}
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          {selectedWeapon ? (
            <div style={{
              ...
              background: 'var(--panel)', border: `1px solid ${primary}`, borderRadius: 4,
            }}>
              <ProceduralIcon
                seed={selectedWeapon.item.name}
                category={resolveItemIconCategory(selectedWeapon.item)}
                color={RARITY_COLOR[selectedWeapon.item.rarity] ?? RARITY_COLOR.common}
                size={20}
              />
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 13 }}>{selectedWeapon.item.name}</span>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: primary }}>
                +{selectedWeapon.item.attackBonus ?? 0} ATK
              </span>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-dim)', marginLeft: 'auto' }}>
                {selectedWeapon.item.maxDurability
                  ? `${selectedWeapon.remainingDurability ?? selectedWeapon.item.maxDurability} use${(selectedWeapon.remainingDurability ?? selectedWeapon.item.maxDurability) === 1 ? '' : 's'} left`
                  : 'Unbreakable'}
              </span>
              <button className="btn ghost" style={{ fontSize: 11, padding: '3px 8px' }} onClick={() => setSelectedWeapon(null)}>
                Unequip
              </button>
            </div>
          ) : (
          ...
          {pickerOpen && (
            <div style={{
              marginTop: 10, display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))',
              gap: 8, maxHeight: 260, overflowY: 'auto', padding: 4,
            }}>
              {weapons.length === 0 && (
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-dim)', gridColumn: '1 / -1' }}>
              ...
              {weapons.map((w) => (
                <button
                  key={w.id}
                  onClick={() => { setSelectedWeapon(w); setPickerOpen(false); }}
                  style={{
                ...
                >
                  <span style={{ fontSize: 16, display: 'flex', alignItems: 'center', gap: 6 }}>
                    <ProceduralIcon
                      seed={w.item.name}
                      category={resolveItemIconCategory(w.item)}
                      color={RARITY_COLOR[w.item.rarity] ?? RARITY_COLOR.common}
                      size={18}
                    />
                    {w.item.name}
                  </span>
                  <span style={{ fontSize: 11, color: primary }}>+{w.item.attackBonus ?? 0} ATK</span>
                  <span style={{ fontSize: 10, color: 'var(--text-dim)' }}>
                    {w.item.maxDurability
                      ? `${w.remainingDurability ?? w.item.maxDurability} use${(w.remainingDurability ?? w.item.maxDurability) === 1 ? '' : 's'} left · x${w.quantity}`
                      : 'Unbreakable'}
                  </span>
                </button>
              ))}
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-dim)', marginBottom: 6 }}>REPS / AMOUNT</div>
            <input
              type="number"
              min={1}
              max={9999}
          ...
          <div style={{ paddingTop: 22 }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-dim)', marginBottom: 4 }}>TOTAL DAMAGE</div>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: 22, color: primary, fontWeight: 700 }}>
              {(amount * ((EXERCISE_TYPES.find(e => e.key === exerciseType)?.damage ?? 0) + (selectedWeapon?.item.attackBonus ?? 0))).toLocaleString()}
            </div>
          </div>
        </div>

        {error && <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--danger)', marginTop: 8 }}>{error}</div>}

        <button
          className="btn-awaken"
          style={{ width: '100%', marginTop: 16, fontSize: 14, padding: '14px', letterSpacing: '0.1em' }}
      ...
      {/* ── BOSS LORE ── */}
      <div className="system-window" style={{ opacity: 0.8 }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-dim)', letterSpacing: '0.15em', marginBottom: 8 }}>
          SYSTEM LOG
        </div>

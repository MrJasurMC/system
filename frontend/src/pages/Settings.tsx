...
export function Settings() {
  ...
  async function onTimezoneChange(timezone: string) {
    ...
    setTzError('');
    try {
      await api.patch(`/characters/${char!.id}/timezone`, { timezone });
  ...
  return (
    <>
      ...
      {knownAccounts.length > 0 && (
        <SystemWindow title="Other Local Accounts" style={{ marginTop: 16 }}>
          {knownAccounts.map((a) => (
            <div key={a.id} className="list-row">
              ...
              <button
                ...
                onClick={async () => {
                  await logout();
                  nav('/login', { state: { prefillEmail: a.email } });
                }}
              >
                Switch
              </button>
      ...
      <SystemWindow title="Region" style={{ marginTop: 16 }}>
        ...
        <select
          className="input"
          value={char?.timezone ?? 'Asia/Tashkent'}
          disabled={tzBusy || !char}
      ...
      <SystemWindow title="New Game" style={{ marginTop: 16, borderColor: 'rgba(220,160,50,0.3)' }}>
      ...
      <SystemWindow title="Danger Zone" style={{ marginTop: 16, borderColor: 'rgba(220,50,50,0.3)' }}>

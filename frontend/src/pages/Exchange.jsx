...
export function Exchange() {
  ...
  async function load() {
    try {
      setItems(await api.get('/inventory/exchange'));
    } catch (err) {
  ...
  return <>
      ...
      marginTop: 24
        ...
        gridColumn: '1 / -1'
      }}>The exchange is currently empty.</div>}
        
        {items.map(item => <SystemWindow key={item.id} title={item.category.toUpperCase()} style={{
        display: 'flex',
            ...
            flex: 1
              ...
              alignItems: 'flex-start'
                ...
                gap: 8
                    ...
                    <ProceduralIcon seed={item.name} category={resolveItemIconCategory(item)} color={RARITY_COLOR[item.rarity] ?? RARITY_COLOR.common} size={24} />
                    {item.name}
                  </h3>
                  <div style={{
                display: 'flex',
              ...
              flexWrap: 'wrap'
                  ...
                  {item.type === 'weapon' && !!item.attackBonus && <span className="badge red">+{item.attackBonus} ATK</span>}
                  {item.type === 'weapon' && <span className="badge" style={{
                opacity: 0.8
              }}>
                      {item.maxDurability && item.maxDurability > 0 ? `${item.maxDurability} use${item.maxDurability === 1 ? '' : 's'}` : 'Unbreakable'}
                    </span>}
                </div>
              ...
              <button className={`btn ${busyId === item.id ? 'ghost' : ''}`} style={{
            width: '100%',
            padding: 12,
            fontSize: 16,
            marginTop: 'auto'
          }} disabled={busyId === item.id || (char?.gold ?? 0) < item.goldPrice} onClick={() => purchase(item.id)}>

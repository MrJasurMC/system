...
function minCompletionDelayMs(estimatedTimeMinutes) {
...
function formatNextAt(iso, timezone) {
  if (!iso) return null;
...
function Countdown({
  ...
  return <span style={{
    ...
    fontSize: 11,
    color: remaining === 'EXPIRED' ? 'var(--danger)' : 'var(--accent-soft)'
  }}>
      ⏱ {remaining}
    </span>;
}
export function Quests() {
  ...
  async function accept(questId) {
    setAcceptingId(questId);
    setAvailableErrors(prev => ({
      ...prev,
      [questId]: ''
    }));
    try {
      ...
      setAvailableErrors(prev => ({
        ...prev,
        [questId]: getErrorMessage(err, 'Failed to accept quest.')
      }));
    } finally {
      setAcceptingId(null);
    }
  }
  async function complete(questId) {
    setBusyId(questId);
    setCardErrors(prev => ({
      ...prev,
      [questId]: ''
    ...
    try {
      ...
      setCardErrors(prev => ({
        ...prev,
        [questId]: getErrorMessage(err, 'Failed to complete quest.')
      }));
    } finally {
      setBusyId(null);
    }
  ...
  const voice = active.filter(m => m.quest.type === 'voice_training');
  ...
  const availableVoice = available.filter(q => q.type === 'voice_training');
  ...
  // Sunday shows neither an active quest nor a "next at" time (nextVoiceQuestAt
  // just points at Monday's reset) — that's the rest day, not a loading gap.
  const nextVoice = voice.length === 0 && availableVoice.length === 0 ? formatNextAt(char?.nextVoiceQuestAt, char?.timezone) : null;
  function QuestCard({
    entry
  }) {
    const q = entry.quest;
    ...
    const {
      tasks
    } = parseMissionTasks(q.description);
    ...
    return <div className="system-window" style={{
      marginBottom: 12,
      border,
      boxShadow: shadow
    }}>
        ...
        marginBottom: 12
          ...
          <div>
            ...
            marginBottom: 6
              ...
              <span className={`badge ${isPunishment ? 'danger' : 'blue'}`} style={{
              fontSize: 10
            }}>
                {TYPE_LABEL[q.type] ?? q.type.toUpperCase()}
              </span>
              <span className={`badge ${DIFFICULTY_COLOR[q.difficulty] ?? 'blue'}`} style={{
              fontSize: 10
            }}>
            ...
            fontSize: 18,
            color: isPunishment ? 'var(--danger)' : 'var(--text)',
          ...
          flexShrink: 0
            ...
            {q.goldReward > 0 && <div className="badge gold" style={{
            fontSize: 11
          }}>+{q.goldReward} GOLD</div>}
          </div>
        </div>

        {q.description && <div style={{
        marginBottom: 12
      }}>
        ...
        alignItems: 'center'
          ...
          flexWrap: 'wrap'
            ...
            {q.attributeReward && Object.entries(q.attributeReward).map(([attr, val]) => val > 0 && <span key={attr} className="badge blue" style={{
            fontSize: 10
          }}>+{val} {attr}</span>)}
          </div>
          {!isCompleted && <button className={isPunishment ? 'btn-awaken' : 'btn'} style={{
          fontSize: 12,
          padding: '8px 16px',
          ...(isPunishment && {
            background: 'linear-gradient(135deg, #8b1a1a, #c0392b)'
          }),
          ...((!allChecked || stillWaiting) && {
            opacity: 0.5,
            cursor: 'not-allowed'
          })
        }} disabled={busyId === entry.questId || !allChecked || stillWaiting} title={!allChecked ? 'Check off every task first' : stillWaiting ? 'The System is still verifying your effort' : undefined} onClick={() => complete(entry.questId)}>
              {busyId === entry.questId ? '...' : stillWaiting ? `Verifying — ${Math.ceil(waitRemainingMs / 1000)}s` : !allChecked ? 'Check off tasks first' : isPunishment ? 'Begin Trial' : 'Mark Complete'}
            </button>}
          {isCompleted && <span className="badge green">COMPLETE</span>}
        </div>
      </div>;
  }
  if (isLoading) {
  ...
  return <>
      ...
      marginBottom: 24
        ...
        marginBottom: 10
          ...
          {[...availableMain, ...availableSide].map(q => <div key={q.id} className="system-window" style={{
        marginBottom: 12
      }}>
              <div style={{
          display: 'flex',
          ...
          marginBottom: 8
              ...
              marginBottom: 6
                ...
                fontSize: 10
              }}>{TYPE_LABEL[q.type] ?? q.type.toUpperCase()}</span>
                    <span className={`badge ${DIFFICULTY_COLOR[q.difficulty] ?? 'blue'}`} style={{
                fontSize: 10
              }}>
                      {q.difficulty?.toUpperCase()}
            ...
            flexShrink: 0
              ...
              {availableErrors[q.id] && <div style={{
          fontFamily: 'var(--font-mono)',
          fontSize: 12,
          color: 'var(--danger)',
          marginBottom: 10
          ...
          justifyContent: 'flex-end'
            ...
            fontSize: 12,
            padding: '8px 16px'
          }} disabled={acceptingId === q.id} onClick={() => accept(q.id)}>
      ...
      marginBottom: 24
        ...
        marginBottom: 10
          ...
          {nextMain && <div className="system-window" style={{
        fontFamily: 'var(--font-mono)',
        fontSize: 12,
        color: 'var(--text-dim)'
      }}>
              Next Main Quest: <b style={{
          color: 'var(--text)'
        }}>{nextMain}</b>
            </div>}
        </div>}
      ...
      marginBottom: 24
        ...
        marginBottom: 10
          ...
          {nextSide && <div className="system-window" style={{
        fontFamily: 'var(--font-mono)',
        fontSize: 12,
        color: 'var(--text-dim)'
      }}>
              Next Side Quest: <b style={{
          color: 'var(--text)'
        }}>{nextSide}</b>
            </div>}
        </div>}
      ...
      {/* ── Voice Training ── */}
      {(voice.length > 0 || nextVoice) && <div style={{
      marginBottom: 24
    }}>
          <div style={{
        fontFamily: 'var(--font-mono)',
        ...
        marginBottom: 10
          ...
          {voice.map(e => <QuestCard key={e.id} entry={e} />)}
          {nextVoice && <div className="system-window" style={{
        fontFamily: 'var(--font-mono)',
        fontSize: 12,
        color: 'var(--text-dim)'
      }}>
              Next Voice Training: <b style={{
          color: 'var(--text)'
        }}>{nextVoice}</b>
            </div>}
        </div>}

      {active.length === 0 && available.length === 0 && !nextMain && !nextSide && !nextVoice && <SystemWindow title="No Active Quests">
          <div className="empty-state">The System has no missions for you right now. Return tomorrow.</div>
        </SystemWindow>}

      {/* ── History ── */}
      {completed.length > 0 && <SystemWindow title={`Completed — ${completed.length}`} style={{
      marginTop: 24,
      opacity: 0.7
    }}>
          {completed.map(e => <div key={e.id} className="list-row">
              <span style={{
          fontSize: 13
        }}>{e.quest.title}</span>
      ...
      {failed.length > 0 && <SystemWindow title={`Failed — ${failed.length}`} style={{
      marginTop: 12,
      opacity: 0.6
    }}>
          {failed.map(e => <div key={e.id} className="list-row">
              <span style={{
          fontSize: 13
        }}>{e.quest.title}</span>

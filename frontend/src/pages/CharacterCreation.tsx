...
export function CharacterCreation() {
  ...
  const [characterClass, setCharacterClass] = useState<(typeof CLASSES)[number]['value']>('warrior');
  ...
  async function onCreateForExistingUser(e: FormEvent) {
    e.preventDefault();
    setError('');
    ...
    setBusy(true);
    try {
      await api.post('/characters', { name: characterName, class: characterClass });
      nav('/');
    } catch (err: unknown) {
  ...
  if (user) {
    return (
      <div className="auth-shell" style={{ background: '#020306' }}>
        <div className="auth-card">
          <div className="auth-title" style={{ color: 'var(--accent)', textShadow: '0 0 10px var(--accent-soft)' }}>
            SYSTEM
          </div>
          <div className="auth-sub" style={{ color: 'var(--text-dim)', letterSpacing: '0.2em' }}>
          ...
          <SystemWindow title="Integration Sequence">
            <form onSubmit={onCreateForExistingUser}>
              ...
              <div className="field">
                <label>Class</label>
                <select value={characterClass} onChange={(e) => setCharacterClass(e.target.value as typeof characterClass)}>
                  {CLASSES.map((c) => (
              ...
              <button className="btn" type="submit" disabled={busy} style={{ width: '100%', marginTop: '10px' }}>
                {busy ? 'Creating...' : 'Begin'}
              </button>
            </form>
          </SystemWindow>
  ...
  async function quickRegister() {
    ...
    try {
      ...
      await api.post('/characters', { name: cn, class: cc });
      nav('/');
    } catch (err: unknown) {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    const validationError =
      ...
      ValidationService.confirmPassword(password, confirmPassword) ||
      (characterName.trim().length < 2 ? 'Character name must be at least 2 characters.' : null);
    if (validationError) {
    ...
    <div className="auth-shell" style={{ background: '#020306' }}>
      <div className="auth-card">
        ...
        <div style={{ textAlign: 'center', marginTop: 16, fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-dim)' }}>
          Already have an account? <Link to="/login" style={{ color: 'var(--accent-soft)' }}>Log in</Link>
        </div>
      </div>
    </div>
  );

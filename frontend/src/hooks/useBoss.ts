...
export function useBoss() {
  ...
  const damageMutation = useMutation({
    mutationFn: ({ bossId, exerciseType, amount, weaponId }: { bossId: string; exerciseType: string; amount: number; weaponId?: string }) => 
      bossApi.damage(bossId, exerciseType, amount, weaponId),
    onSuccess: (updatedBoss) => {
      queryClient.setQueryData(['boss', 'status'], { active: true, boss: updatedBoss });
    }
  });
